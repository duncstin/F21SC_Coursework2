...
def get_file_generator(file, doc_id='', user_id=''):
...
def also_likes(file, doc_id, user_id=''):
    get_readers_and_documents(file, doc_id, user_id)


def get_readers_and_documents(file, doc_id, user_id):
    file_as_generator = get_file_generator(file, doc_id)
    readers = []
    for line in file_as_generator:
        reader = process_line(line, "visitor_uuid")
        if reader:
            readers.append(reader)
    docs_for_readers = all_reader_docs(file, readers)
...
def also_likes_graph(file, doc_id, user_id=''):
    info = get_readers_and_documents(file, doc_id, user_id)
    dot = Digraph(name="Also likes", node_attr={'shape': 'plaintext', 'fontsize': '16'})
    find = re.compile("_(.*?)_")
    try:
        first = re.search(find, file).group(1)
        scale = "Size: " + first
...
also_likes_graph('sample_3M_lines.json', '140109173556-a4b921ab7619621709b098aa9de4d736')
